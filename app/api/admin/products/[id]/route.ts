...
export async function PATCH(req: Request, ctx: { params: { id: string } }) {
  ...
  const keepGalleryRaw = String(fd.get("keep_gallery_urls") ?? "[]");
  let keepGallery: string[] = [];
  try {
    const parsed = JSON.parse(keepGalleryRaw);
    if (Array.isArray(parsed)) keepGallery = parsed.filter((x) => typeof x === "string");
  } catch {
    keepGallery = [];
  }

  const mainImage = fd.get("main_image");
  const newGallery = fd.getAll("gallery_images");
  ...
  try {
    ...
    const gallery_urls: string[] = [...keepGallery];
